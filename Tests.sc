...
val liast = List((1, "Das ist ein geiler Text"),(2, "Der hier weitergeht der"),(3, "Und endet hier"))
                                                  //> liast  : List[(Int, String)] = List((1,Das ist ein geiler Text), (2,Der hier
                                                  //|  weitergeht der), (3,Und endet hier))
...
def createInverseIndex(l:List[(Int,String)]):Map[String,List[Int]]={
   ...
   createInverseIndex(getAllWordsWithIndex(liast))//> res0: Map[String,List[Int]] = Map(und -> List(3), geiler -> List(1), hier -
                                                  //| > List(3, 2), ist -> List(1), das -> List(1), weitergeht -> List(2), der ->
                                                  //|  List(2, 2), ein -> List(1), endet -> List(3), text -> List(1))
